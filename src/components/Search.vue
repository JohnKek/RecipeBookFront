...
export default {
  ...
  methods: {
    async loadRecipe() {
      ...
      let searchUrl=''
      if(name!=''){
       searchUrl= 'http://localhost:8080/recipe/Test/name/'+name
    }
      else{
        searchUrl='http://localhost:8080/recipe/all'
    }
      var productApi = Vue.resource(searchUrl);
      productApi.get().then(result =>
          result.json().then(data =>
              data.forEach(recipe => this.recipes.push(recipe))
          )
      )
      this.recipes.forEach(recipe => console.log(recipe.idRecipe))
