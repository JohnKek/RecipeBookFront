      ...
      <div class="main_block_container container">
        <div class="main_block_body">
          ...
          <div class="recipe_item">
            <img class="recipe_pic" :src="require(`../assets/${info.recipeImgPath}`)" alt="">
            <div class="recipe_time">
              <div class="time_pic"></div>
              <div class="time_text">{{ info.recipeTime }} минуты</div>
            </div>
<!--            <div class="recipe_name">{{ info.recipe_name }}</div>-->
            <div class="recipe_name">{{ info.recipeName }}</div>
          </div>
          <div class="ingridient_block">
            <div class="cook_title">Ингредиенты</div>
            <ul class="ingridient_list" v-for="ingrid in info.ingridientList" :key="ingrid.id">
              <li class="ingridient_item">{{ ingrid.product.name }} — {{ ingrid.weight }} г/мл/шт</li>
            </ul>
          </div>
          <div class="cook_desk_block">
            <div class="cook_title">Способ приготовления</div>
            <div class="cook_step_list" type="1">
              <div class="step_item">{{ info.recipeDesc }}</div>
            </div>
          </div>
        </div>
        <hr>
        <div class="button_block">
          <router-link :to="{name:'main'}" ><input type="button" class="form_button" @click="snos" value="      Удалить рецепт      "/></router-link>
        </div>
        <hr>
        <div class="button_block">
          <router-link :to="{name: 'change',params:{id: idTo}}"><input type="button" class="form_button" value="Редактировать рецепт "/></router-link>
        </div>
      </div>
    </main>
  </div>
...
export default {
  ...
  methods: {
    load() {
      this.idTo = this.$route.params.id
    ...
    snos() {
      let id = this.$route.params.id
      var productApi = Vue.resource('http://localhost:8080/recipe{/id}');
      productApi.remove({id: id})
    }
  }
}
</script>
